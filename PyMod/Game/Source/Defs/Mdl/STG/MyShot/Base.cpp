...
// コンストラクタ
Base::Base( const Vector2DF &pos, int radius, float baseDamage )
...
{
	typedef Game::Util::Ptr<Game::Mdl::STG::Enemy::IEnemy>::Shared PEnemy;
	auto pEnemies = STGControl::getActors()->DetectEnemyHitAll( mHitRect );
	if( !pEnemies->empty() )
	{
